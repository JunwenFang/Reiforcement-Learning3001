...
Usage:
  ...
  main.py selfplay dqn_train_tf [options]
  main.py selfplay dqn_play_tf [options]
...
def command_line_parser():
    ...
    if args['selfplay']:
        ...
        elif args['dqn_train_tf']:
            runner.dqn_train_tf(model_name)
        elif args['dqn_play_tf']:
            runner.dqn_play_tf(model_name)
...
class SelfPlay:
    ...
    def dqn_train_tf(self, model_name):
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_tf_dqn import Player as DQNPlayer
        from agents.agent_random import Player as RandomPlayer
        ...
        env.add_player(RandomPlayer())
        env.add_player(RandomPlayer())
        env.add_player(PlayerShell(name='tf-dqn', stack_size=self.stack))

        env.reset()

        tfdqn = DQNPlayer(env=env)
        tfdqn.initiate_agent(env)
        tfdqn.train()

    def dqn_play_tf(self, model_name):
        from agents.agent_consider_equity import Player as EquityPlayer
        from agents.agent_tf_dqn import Player as DQNPlayer
        from agents.agent_random import Player as RandomPlayer
        ...
        tfdqn = DQNPlayer(load_model=model_name, env=env)
        tfdqn.play(nb_episodes=self.num_episodes, render=self.render)
